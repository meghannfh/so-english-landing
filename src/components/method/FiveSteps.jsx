...
export default function FiveSteps() {
    return (
        ...
        <div className='px-6 py-24 md:px-24 lg:px-56'>
            <p className="mt-6 text-2xl leading-relaxed md:leading-loose md:text-lg font-medium text-slate-900">これは一般的に知られているリスニングとスピーキング時の脳のプロセスを5段階に分けた最も簡素化された図です。上段はリスニング、下段はスピーキングの時の脳のプロセスです。会話はリスニングとスピーキングの連続ですので、これを繰り返します。<br/>
            「アポー→apple」と書いてあるリスニングのステップ1は「音声知覚」といって、言語としての音を認識します。「アポー」と聞いて「apple」と頭の中に英語のスペルを思い浮かべるということです。<br />
            「apple→🍎」と書いてあるリスニングのステップ2は「意味理解」といって、頭の中に浮かべた「apple」は「リンゴ」という意味だと理解することです。<br/>
            「❤️🍊」が描いてあるスピーキングのステップ1は「概念化」といって、自分が言いたいことを整理することです。
            </p>
        </div>
    </div>
    )
